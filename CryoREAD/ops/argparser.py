...
def argparser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Use strict/soft rules to assemble collected fragments in DP step. (Integer), Default value: 0 (strict rules)",
    )
    ...
    parser.add_argument(
        "--resolution", type=float, default=2.5, help="resolution of maps, used for final structure refinement. (Float), Default value: 2.5"
    )
    parser.add_argument(
        "--num_workers", type=int, default=4, help="number of workers to fetch data for GPU inference. (Integer), Default value: 4"
    )
    parser.add_argument(
        "--prediction_only",
        action="store_true",
        help="Optional input. Only run the deep learning prediction step. (True/False) Default value: False",
    )
    parser.add_argument(
        "--no_seqinfo",
        action="store_true",
        help="Optional input. Build structures when no sequence information is available. (True/False) Default value: False",
    )
    parser.add_argument(
        "--refine", action="store_true", help="Optional Input. Use Phenix to do the last step refinement or not (Suggested to set as True)."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Use multiple threads for fragment-based sequence assignment,default:1 (multi-threading is disabled)",
    )
