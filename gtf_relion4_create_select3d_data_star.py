...
def gtf_create_relion4_data_particles_dict():
    ...
    relion_data_particles_dict['_rlnMicrographName'] = [-1,
                                                        '#     Micrograph Name        := %2d (%s)']  # Micrograph, CTF, Window, Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnOpticsGroup'] = [-1,
                                                     '#     Optics Group ID        := %2d (%s)']  # Global except Coordinate Star File
    relion_data_particles_dict['_rlnDefocusU'] = [-1,
                                                  '#     Defocus U              := %2d (%s)']  # CTF, Window, Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnDefocusV'] = [-1,
                                                  '#     Defocus V              := %2d (%s)']  # CTF, Window, Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnDefocusAngle'] = [-1,
                                                      '#     Defocus Angle          := %2d (%s)']  # CTF, Window, Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnAmplitudeContrast'] = [-1,
                                                           '#     Amp. Contrast          := %2d (%s)']  # CTF, Window, Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnPhaseShift'] = [-1,
                                                    '#     Phase shift            := %2d (%s)']  # CTF, Window, Class2d, Class3d    # For Volta Phase Support
    relion_data_particles_dict['_rlnVoltage'] = [-1,
                                                 '#     Acc. Vol.              := %2d (%s)']  # CTF, Window, Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnSphericalAberration'] = [-1,
                                                             '#     Cs                     := %2d (%s)']  # CTF, Window, Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnCtfFigureOfMerit'] = [-1,
                                                          '#     CTF Fig. of Merit      := %2d (%s)']  # CTF, Window, Class2d, Class3d, Refine3d
    ...
    relion_data_particles_dict['_rlnImageName'] = [-1,
                                                   '#     Particle Source        := %2d (%s)']  # Window, Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnCoordinateX'] = [-1,
                                                     '#     X Coordinate           := %2d (%s)']  # Window, Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnCoordinateY'] = [-1,
                                                     '#     Y Coordinate           := %2d (%s)']  # Window, Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnAutopickFigureOfMerit'] = [-1,
                                                               '#     Figure of Merit        := %2d (%s)']  # Only Relion 3.1.0! Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnOriginXAngst'] = [-1,
                                                      '#     X Translation (v3.1.0) := %2d (%s)']  # Only Relion 3.1.0! Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnOriginYAngst'] = [-1,
                                                      '#     Y Translation (v3.1.0) := %2d (%s)']  # Only Relion 3.1.0! Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnAngleRot'] = [-1,
                                                  '#     Rotation               := %2d (%s)']  # Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnAngleTilt'] = [-1,
                                                   '#     Tilt                   := %2d (%s)']  # Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnAnglePsi'] = [-1,
                                                  '#     Psi                    := %2d (%s)']  # Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnMaxValueProbDistribution'] = [-1,
                                                                  '#     Max Probability        := %2d (%s)']  # Class2d, Class3d, Refine3d
    relion_data_particles_dict['_rlnNormCorrection'] = [-1,
                                                        '#     Norm Correction        := %2d (%s)']  # Class2d, Class3d, Refine3d
...
# ++++++++++++++++++++++++++++++
#
def run(relion_particles_data_star_file_rpath, relion_job_dir_rpath, selected_class_id,
        selected_relion_particles_data_star_file_name, relion_project_dir_fpath=None):
    ...
    gtf_print('[GTF_DEBUG] selected_relion_particles_data_star_file_name = {}'.format(
        selected_relion_particles_data_star_file_name))
    ...
    if (not os.path.exists(relion_particles_data_star_file_rpath)):
        gtf_print('[GTF_ERROR] Input RELION particles data star file "{}" is not found.'.format(
            relion_particles_data_star_file_rpath))
        return
    ...
    if selected_class_id < 1:
        gtf_print('[GTF_ERROR] Specified Selected Class ID "{}" is not valid. The value must be greater than 0.'.format(
            selected_class_id))
        return
    ...
    assert os.path.exists(
        relion_particles_data_star_file_rpath), '# Logical Error: Input RELION STAR file must exits at this point of code.'
    file_relion_star = open(relion_particles_data_star_file_rpath, 'r')

    file_path_selected_relion_data_star = os.path.join(relion_job_dir_rpath,
                                                       selected_relion_particles_data_star_file_name)
    ...
    for i_line, str_line in enumerate(file_relion_star):
        ...
        # Then, ignore loop_ in STAR file
            ...
            assert len(
                relion_data_particles_dict) > 0, '# Logical Error: relion_data_particles_dict has not been set properly.'
            assert len(
                relion_category_dict) > 0, '# Logical Error: relion_data_particles_dict has not been set properly.'
            ...
            if str_line.find('_rln') != -1:
                ...
                if relion_key in list(relion_data_particles_dict.keys()):
                    ...
                    gtf_print(relion_data_particles_dict[relion_key][idx_data_title] % (
                    relion_data_particles_dict[relion_key][idx_data_col], relion_key))
            ...
            # Then, read the data entries
                ...
                if i_relion_data_particles_entry % 1000 == 0:
                    gtf_print('# Processing RELION entries from %7d to %7d ...' % (
                    i_relion_data_particles_entry, i_relion_data_particles_entry + 1000 - 1))
                ...
                assert relion_category_dict['class'][
                ...
                for relion_data_particles_key in list(relion_data_particles_dict.keys()):
                    ...
                    particles_data_entry_dict[relion_particle_name][relion_data_particles_key] = tokens_line[
                        relion_data_particles_dict[relion_data_particles_key][idx_data_col] - 1]
    ...
    if is_success:
        ...
        gtf_print(
            '# Selected RELION entry counts                      := {} '.format(i_relion_data_particles_entry_selected))
        gtf_print('# Discarded RELION entry counts                     := {} '.format(
            i_relion_data_particles_entry - i_relion_data_particles_entry_selected))
...
if __name__ == '__main__':
    ...
    if len(args) != 4:
        ...
        gtf_print(
            "[GTF_ERROR] Missing paths to input RELION DATA STAR file and output directory. Please see usage information above")
    ...
    run(args_file_path_relion_star, args_dir_path_work, args_selected_class_id, args_file_name_selectd_star,
        options_dir_path_relion_project)
